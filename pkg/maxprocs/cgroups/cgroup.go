...
func (err cgroupSubsysFormatInvalidError) Error() string {
	return fmt.Sprintf("cgroups: invalid format for Subsystem: %q", err.line)
}

func (err mountPointFormatInvalidError) Error() string {
	return fmt.Sprintf("cgroups: invalid format for MountPoint: %q", err.line)
}

func (err pathNotExposedFromMountPointError) Error() string {
	return fmt.Sprintf("cgroups: path %q is not a descendant of mount point root %q and cannot be exposed from %q", err.path, err.root, err.mountPoint)
}
